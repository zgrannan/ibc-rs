...
impl ClientDef for AnyClient {
    ...
    fn verify_upgrade_and_update_state(
        ...
        match self {
            Self::Tendermint(client) => {
                let (new_client_state, new_consensus_state) = downcast!(
                    new_client_state => AnyClientState::Tendermint,
                    new_consensus_state => AnyConsensusState::Tendermint,
                )
                .ok_or_else(|| Kind::ClientArgsTypeMismatch(ClientType::Tendermint))?;
            ...
            Self::Mock(client) => {
                let (new_client_state, new_consensus_state) = downcast!(
                    new_client_state => AnyClientState::Mock,
                    new_consensus_state => AnyConsensusState::Mock,
                )
                .ok_or_else(|| Kind::ClientArgsTypeMismatch(ClientType::Mock))?;
