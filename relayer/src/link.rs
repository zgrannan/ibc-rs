...
use crate::chain::{
    counterparty::check_channel_counterparty, requests::QueryConnectionRequest,
};
...
impl<ChainA: ChainHandle, ChainB: ChainHandle> Link<ChainA, ChainB> {
    ...
    pub fn new_from_opts(
        ...
        {
            return Err(
                LinkError::invalid_channel_state(a_channel_id.clone(), a_chain.id()),
            );
        }
        let b_channel_id = a_channel
            .counterparty()
            .channel_id()
            .ok_or_else(|| LinkError::counterparty_channel_not_found(
                a_channel_id.clone(),
            ))?;
        if a_channel.connection_hops().is_empty() {
            return Err(LinkError::no_connection_hop(a_channel_id.clone(), a_chain.id()));
        }
        ...
        if !a_connection.state_matches(&ConnectionState::Open) {
            return Err(
                LinkError::channel_not_opened(a_channel_id.clone(), a_chain.id()),
            );
        }
        let channel = Channel {
