...
use crate::{
    chain::{
        cosmos::CosmosSdkChain, handle::ChainHandle, runtime::ChainRuntime, ChainType,
    },
    config::Config, error::Error as RelayerError,
};
#[cfg(test)]
use crate::chain::mock::MockChain;
define_error! {
    SpawnError { Relayer[RelayerError] | _ | { "relayer error" }, RuntimeNotFound | _ | {
    "expected runtime to be found in registry" }, MissingChainConfig { chain_id : ChainId
    } | e | { format_args!("missing chain config for '{}' in configuration file", e
    .chain_id) } }
}
impl SpawnError {
