...
pub mod max_msg_num {
    flex_error::define_error! {
        Error { TooSmall { value : usize } | e | {
        format_args!("`max_msg_num` must be greater than or equal to {}, found {}",
        MaxMsgNum::MIN_BOUND, e.value) }, TooBig { value : usize } | e | {
            ...
            D: Deserializer<'de>,
        {
            ...
            MaxMsgNum::new(value)
                .map_err(|e| match e.detail() {
                    ErrorDetail::TooSmall(_) => {
                        D::Error::invalid_value(
                            Unexpected::Unsigned(value as u64),
                            &format!(
                                "a usize greater than or equal to {}", Self::MIN_BOUND
                            )
                                .as_str(),
                        )
                    }
                    ErrorDetail::TooBig(_) => {
                        D::Error::invalid_value(
                            Unexpected::Unsigned(value as u64),
                            &format!("a usize less than or equal to {}", Self::MAX_BOUND)
                                .as_str(),
                        )
                    }
                })
...
pub mod max_tx_size {
    ...
    impl MaxTxSize {
        const DEFAULT: usize = 2 * 1048576;
        const MAX_BOUND: usize = 8 * 1048576;
        #[prusti_contracts::trusted]
        pub fn new(value: usize) -> Result<Self, Error> {
            if value > Self::MAX_BOUND {
                return Err(Error::too_big(value));
...
pub mod memo {
    ...
    impl<'de> Deserialize<'de> for Memo {
            ...
            D: Deserializer<'de>,
        {
            ...
            Memo::new(value)
                .map_err(|e| match e.detail() {
                    ErrorDetail::TooLong(sub) => {
                        D::Error::invalid_length(
                            sub.length,
                            &format!("a string length of at most {}", Self::MAX_LEN)
                                .as_str(),
                        )
                    }
                })
