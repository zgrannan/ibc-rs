...
use crate::{
    ...
    object::Object, registry::{Registry, SharedRegistry},
    rest, supervisor::scan::ScanMode, telemetry,
    util::{lock::LockExt, task::{spawn_background_task, Next, TaskError, TaskHandle}},
    worker::WorkerMap,
};
...
pub fn spawn_supervisor_tasks<Chain: ChainHandle>(
    ...
    let scan = chain_scanner(
            ...
            if options.force_full_scan { ScanMode::Full } else { ScanMode::Auto },
        )
        .scan_chains();
    info!("Scanned chains:");
    info!("{}", scan);
...
fn relay_on_object<Chain: ChainHandle>(
    ...
    let client_filter_outcome = match object {
        Object::Client(client) => {
            client_state_filter.control_client_object(registry, client)
        }
        Object::Connection(conn) => {
            client_state_filter.control_conn_object(registry, conn)
        }
        Object::Channel(chan) => client_state_filter.control_chan_object(registry, chan),
        Object::Packet(packet) => {
            client_state_filter.control_packet_object(registry, packet)
        }
    ...
    match client_filter_outcome {
        ...
        Ok(Permission::Deny) => {
            warn!("client filter denies relaying on object {}", object.short_name());
            false
        }
        Err(e) if e.log_as_debug() => {
            debug!(
                "denying relaying on object {}, caused by: {}", object.short_name(), e
            );
            false
        }
        Err(e) => {
    ...
    F: FnOnce() -> Option<Object>,
{
    if enabled {
        if let Some(object) = object_ctor() {
            collected.per_object.entry(object).or_default().push(event.clone());
        }
    }
}
#[prusti_contracts::trusted]
pub fn collect_events(
    ...
    let mut collected = CollectedEvents::new(
        batch.height,
        ...
        match event {
            ...
            IbcEvent::UpdateClient(ref update) => {
                collect_event(
                    ...
                    || {
                        if let Ok(object)
                            = Object::for_update_client(update, src_chain) {
                ...
                collect_event(
                    ...
                    || {
                        event
                            ...
                            .and_then(|attr| {
                                Object::connection_from_conn_open_events(attr, src_chain)
                                    .ok()
                            })
                    },
                );
        ...
        match chain {
            Ok(chain) => {
                match chain.health_check() {
                    ...
                    Err(e) => {
                        error!(chain = % id, "failed to perform health check: {}", e)
                    }
                }
            }
            Err(e) => {
...
fn init_subscriptions<Chain: ChainHandle>(
    ...
    for chain_config in chains {
        let chain = match registry.get_or_spawn(&chain_config.id) {
            Ok(chain) => chain,
            Err(e) => {
                error!("failed to spawn chain runtime for {}: {}", chain_config.id, e);
                continue;
            }
        };
        match chain.subscribe() {
            ...
            Err(e) => {
                error!("failed to subscribe to events of {}: {}", chain_config.id, e)
            }
        }
    }
    if registry.size() == 0 {
...
fn process_batch<Chain: ChainHandle>(
    ...
    for (object, events) in collected.per_object.into_iter() {
        ...
        let src = registry.get_or_spawn(object.src_chain_id()).map_err(Error::spawn)?;
        let dst = registry.get_or_spawn(object.dst_chain_id()).map_err(Error::spawn)?;
        if let Object::Packet(_path) = object.clone() {
            telemetry!(
                { for e in events.clone() { match e {
                IbcEvent::SendPacket(send_packet_ev) => { ibc_telemetry::global()
                .send_packet_count(send_packet_ev.packet.sequence.into(), send_packet_ev
                .height().revision_height(), & src.id(), & _path.src_channel_id, & _path
                .src_port_id, & dst.id(),); }
                IbcEvent::WriteAcknowledgement(write_ack_ev) => { ibc_telemetry::global()
                .acknowledgement_count(write_ack_ev.packet.sequence.into(), write_ack_ev
                .height().revision_height(), & dst.id(), & _path.src_channel_id, & _path
                .src_port_id, & src.id(),); } _ => {} } } }
            );
        }
...
fn handle_batch<Chain: ChainHandle>(
    ...
    match batch.deref() {
        ...
        Err(EventError(EventErrorDetail::SubscriptionCancelled(_), _)) => {
            warn!(
                chain.id = % chain_id,
                "event subscription was cancelled, clearing pending packets"
            );
            let _ = clear_pending_packets(workers, &chain_id)
                .map_err(|e| {
                    error!("[{}] error during clearing pending packets: {}", chain_id, e)
                });
        }
        Err(e) => error!("[{}] error in receiving event batch: {}", chain_id, e),
    }
}
