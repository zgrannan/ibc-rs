...
use crate::{
    config::Config, rest::request::ReplySender, rest::request::{Request, VersionInfo},
...
pub const NAME: &str = env!(
    "CARGO_PKG_NAME", "the env. variable CARGO_PKG_NAME in ibc-relayer is not set!"
);
pub const VER: &str = env!(
    "CARGO_PKG_VERSION", "the env. variable CARGO_PKG_VERSION in ibc-relayer is not set!"
);
            ...
            match request {
                Request::Version { reply_to } => {
                    ...
                    reply_to
                        .send(Ok(v))
                        .unwrap_or_else(|e| {
                            error!("error replying to a REST request {}", e)
                        });
                }
