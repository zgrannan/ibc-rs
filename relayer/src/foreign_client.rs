...
define_error! {
    ForeignClientError { ClientCreate { chain_id : ChainId, description : String }
    [RelayerError] | e | {
    format_args!("error raised while creating client for chain {0}: {1}", e.chain_id, e
    .description) }, Client[ClientError] | _ | { "ICS02 client error" },
    HeaderInTheFuture { src_chain_id : ChainId, src_header_height : Height,
    src_header_time : Timestamp, dst_chain_id : ChainId, dst_latest_header_height :
    Height, dst_latest_header_time : Timestamp, max_drift : Duration } | e | {
    ...
    e.src_chain_id, e.src_header_height, e.src_header_time, e.dst_chain_id, e
    .dst_latest_header_height, e.dst_latest_header_time, e.max_drift) }, ClientUpdate {
    chain_id : ChainId, description : String } [RelayerError] | e | {
    format_args!("error raised while updating client on chain {0}: {1}", e.chain_id, e
    .description) }, ClientUpdateTiming { chain_id : ChainId, clock_drift : Duration,
    description : String } [TimestampOverflowError] | e | {
    format_args!("error raised while updating client on chain {0}: {1}", e.chain_id, e
    .description) }, ClientAlreadyUpToDate { client_id : ClientId, chain_id : ChainId,
    height : Height, } | e | {
    format_args!("Client {} is already up-to-date with chain {}@{}", e.client_id, e
    .chain_id, e.height) }, MissingSmallerTrustedHeight { chain_id : ChainId,
    target_height : Height, } | e | {
    format_args!("chain {} is missing trusted state smaller than target height {}", e
    .chain_id, e.target_height) }, MissingTrustedHeight { chain_id : ChainId,
    target_height : Height, } | e | {
    format_args!("chain {} is missing trusted state at target height {}", e.chain_id, e
    .target_height) }, ClientRefresh { client_id : ClientId, reason : String }
    [RelayerError] | e | {
    format_args!("error raised while trying to refresh client {0}: {1}", e.client_id, e
    .reason) }, ClientQuery { client_id : ClientId, chain_id : ChainId, } [RelayerError]
    | e | { format_args!("failed while querying for client {0} on chain id {1}", e
    .client_id, e.chain_id) }, ClientConsensusQuery { client_id : ClientId, chain_id :
    ChainId, height : Height } [RelayerError] | e | {
    format_args!("failed while querying for client consensus state {0} on chain id {1} for height {2}",
    e.client_id, e.chain_id, e.height) }, ClientUpgrade { client_id : ClientId, chain_id
    : ChainId, description : String, } [RelayerError] | e | {
    format_args!("failed while trying to upgrade client id {0} for chain {1}: {2}: {3}",
    e.client_id, e.chain_id, e.description, e.source) }, ClientUpgradeNoSource {
    client_id : ClientId, chain_id : ChainId, description : String, } | e | {
    format_args!("failed while trying to upgrade client id {0} for chain {1}: {2}", e
    .client_id, e.chain_id, e.description) }, ClientEventQuery { client_id : ClientId,
    chain_id : ChainId, consensus_height : Height } [RelayerError] | e | {
    format_args!("failed while querying Tx for client {0} on chain id {1} at consensus height {2}",
    e.client_id, e.chain_id, e.consensus_height) }, UnexpectedEvent { client_id :
    ClientId, chain_id : ChainId, event : String, } | e | {
    format_args!("failed while querying Tx for client {0} on chain id {1}: query Tx-es returned unexpected event: {2}",
    e.client_id, e.chain_id, e.event) }, MismatchChainId { client_id : ClientId,
    expected_chain_id : ChainId, actual_chain_id : ChainId, } | e | {
    format_args!("failed while finding client {0}: expected chain_id in client state: {1}; actual chain_id: {2}",
    e.client_id, e.expected_chain_id, e.actual_chain_id) }, ExpiredOrFrozen { client_id :
    ClientId, chain_id : ChainId, description : String, } | e | {
    format_args!("client {0} on chain id {1} is expired or frozen: {2}", e.client_id, e
    .chain_id, e.description) }, ConsensusStateNotTrusted { height : Height, elapsed :
    Duration, } | e | {
    format_args!("the consensus state at height {} is outside of trusting period: elapsed {:?}",
    e.height, e.elapsed) }, Misbehaviour { description : String, } [RelayerError] | e | {
    format_args!("error raised while checking for misbehaviour evidence: {0}", e
    .description) }, MisbehaviourExit { reason : String } | e | {
    format_args!("cannot run misbehaviour: {0}", e.reason) }, SameChainId { chain_id :
    ChainId } | e | {
    format_args!("the chain ID ({}) at the source and destination chains must be different",
    e.chain_id) }, MissingClientIdFromEvent { event : IbcEvent } | e | {
    format_args!("cannot extract client_id from result: {:?}", e.event) },
    ChainErrorEvent { chain_id : ChainId, event : IbcEvent } | e | {
...
for ForeignClient<DstChain, SrcChain> {
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "{} -> {}:{}", self.src_chain.id(), self.dst_chain.id(), self.id)
    }
}
impl<DstChain: ChainHandle, SrcChain: ChainHandle> ForeignClient<DstChain, SrcChain> {
            ...
            )
        {
            Ok((cs, _)) => {
                if cs.chain_id() != expected_target_chain.id() {
                    Err(
                        ForeignClientError::mismatch_chain_id(
                            client_id.clone(),
            ...
            Err(e) => {
                Err(
                    ForeignClientError::client_query(
                        client_id.clone(),
                        host_chain.id(),
                        e,
                    ),
    ...
    pub fn upgrade(
        ...
        let mut msgs = self
            ...
            .map_err(|_| {
                ForeignClientError::client_upgrade_no_source(
                    ...
                    self.dst_chain.id(),
                    format!(
                        "is chain {} halted at height {}?", self.src_chain().id(),
                        src_upgrade_height
                    ),
                )
            })?;
        ...
        debug!("[{}]  upgraded client consensus state {:?}", self, consensus_state);
        let signer = self
            .dst_chain
            .get_signer()
            .map_err(|e| {
    ...
    pub fn build_create_client(
        ...
        let signer = self
            ...
            .map_err(|e| {
                ForeignClientError::client_create(
                    self.src_chain.id(),
                    format!(
                        "failed while fetching the dst chain ({}) signer", self.dst_chain
                        .id()
                    ),
                    e,
                )
        ...
        let src_config = self
            ...
            .map_err(|e| {
                ForeignClientError::client_create(
                    self.src_chain.id(),
                    "failed while querying the source chain for configuration"
                        .to_string(),
                    e,
                )
            })?;
        let dst_config = self
            ...
            .map_err(|e| {
                ForeignClientError::client_create(
                    self.dst_chain.id(),
                    "failed while querying the destination chain for configuration"
                        .to_string(),
                    e,
                )
            })?;
        ...
        let consensus_state = self
            ...
            .map_err(|e| {
                ForeignClientError::client_create(
                    self.src_chain.id(),
                    "failed while building client consensus state from src chain"
                        .to_string(),
                    e,
                )
            })?
    ...
    pub fn build_create_client_and_send(
        ...
        let res = self
            ...
            .send_messages_and_wait_commit(
                TrackedMsgs::new_single(new_msg.to_any(), "create client"),
            )
            .map_err(|e| {
                ForeignClientError::client_create(
                    self.dst_chain.id(),
    ...
    pub fn validated_client_state(
        ...
        match self
            .check_consensus_state_trusting_period(
                &client_state,
                &client_state.latest_height(),
            )?
        {
            ConsensusStateTrusted::NotTrusted {
                ...
                error!(
                    latest_height = % client_state.latest_height(), network_timestmap = %
                    network_timestamp, consensus_state_timestamp = %
                    consensus_state_timestmap, elapsed = ? elapsed,
            ...
            ConsensusStateTrusted::Trusted { elapsed } => {
    ...
    fn check_consensus_state_trusting_period(
        ...
        let _span = span!(
            Level::DEBUG, "check_consensus_state_trusting_period", height = % height
        )
            .entered();
        if client_state.chain_id() != self.src_chain.id() {
            warn!(
                "the chain id in the client state ('{}') is inconsistent with the client's source chain id ('{}')",
        ...
        if client_state.expired(elapsed) {
            ...
            Ok(ConsensusStateTrusted::Trusted {
                elapsed,
            })
        }
    }
    ...
    fn solve_trusted_height(
        ...
        if client_latest_height < target_height {
            ...
            warn!(
                "[{}] resolving trusted height from the full list of consensus state heights for target height {}; this may take a while",
                self, target_height
            );
    ...
    fn wait_for_header_validation_delay(
        ...
        let ts_adjusted = (status.timestamp + client_state.max_clock_drift())
            .map_err(|e| {
                ForeignClientError::client_update_timing(
                    self.dst_chain.id(),
                    client_state.max_clock_drift(),
                    "failed to adjust timestamp of destination chain with clock drift"
                        .to_string(),
                    e,
                )
            })?;
        if header.timestamp().after(&ts_adjusted) {
            warn!(
                "[{}] src header {} is after dst latest header {} + client state drift {:?}, wait for next height on {}",
                self, header.timestamp(), status.timestamp, client_state
                .max_clock_drift(), self.dst_chain().id()
            );
            ...
            loop {
                ...
                status = self
                    ...
                    .map_err(|e| {
                        ForeignClientError::client_update(
                            self.dst_chain.id(),
                            "failed querying latest status of the destination chain"
                                .to_string(),
                            e,
                        )
                    })?;
        ...
        let next_ts_adjusted = (status.timestamp + client_state.max_clock_drift())
            .map_err(|e| {
                ForeignClientError::client_update_timing(
                    self.dst_chain.id(),
                    client_state.max_clock_drift(),
        ...
        if header.timestamp().after(&next_ts_adjusted) {
            Err(
                ForeignClientError::header_in_the_future(
                    self.src_chain.id(),
                    header.height(),
                    header.timestamp(),
                    self.dst_chain.id(),
    ...
    pub fn build_update_client_with_trusted(
        ...
        if trusted_height != client_state.latest_height() {
            ...
            {
                error!(
                    trusted_height = % trusted_height, network_timestmap = %
                    network_timestamp, consensus_state_timestamp = %
                    consensus_state_timestmap, elapsed = ? elapsed,
                ...
                return Err(
                    ForeignClientError::consensus_state_not_trusted(
                        trusted_height,
                        elapsed,
                    ),
                );
        ...
        for header in support {
            debug!(
                "[{}] MsgUpdateAnyClient for intermediate height {}", self, header
                .height(),
            );
            msgs.push(
                MsgUpdateAnyClient {
        ...
        debug!(
            "[{}] MsgUpdateAnyClient from trusted height {} to target height {}", self,
    ...
    pub fn build_update_client_and_send(
        ...
        let new_msgs = self
            .wait_and_build_update_client_with_trusted(target_height, trusted_height)?;
        if new_msgs.is_empty() {
            return Err(
                ForeignClientError::client_already_up_to_date(
                    self.id.clone(),
                    self.src_chain.id(),
                    target_height,
                ),
    ...
    pub fn update_client_event(
        ...
        for i in 0..MAX_RETRIES {
            ...
            match result {
                Err(e) => {
                    error!(
                        "[{}] query_tx with error {}, retry {}/{}", self, e, i + 1,
                        MAX_RETRIES
                    );
                    continue;
                }
        ...
        let update = downcast!(event.clone() => IbcEvent::UpdateClient)
            .ok_or_else(|| {
    ...
    pub fn detect_misbehaviour(
        ...
        let _span = span!(
            tracing::Level::DEBUG, "detect_misbehaviour", update_height = ? span_guard,
        )
            .entered();
        let (client_state, _) = {
            self.dst_chain()
        ...
        trace!(
            ...
            consensus_state_heights.iter().take(50).join(", "), consensus_state_heights
            .len()
        );
        ...
        for target_height in consensus_state_heights {
            ...
            if update_event.header.is_none() {
                return Err(
                    ForeignClientError::misbehaviour_exit(
                        format!(
                            "could not extract header from update client event {:?} emitted by chain {:?}",
                            update_event, self.dst_chain.id()
                        ),
                    ),
                );
            }
                    ...
                    return Err(
                        ForeignClientError::misbehaviour(
                            format!(
                                ...
                                update_event.client_id(), update_event.consensus_height(),
                            ),
                            e,
                        ),
                    );
            ...
            if start_time.elapsed() > MAX_MISBEHAVIOUR_CHECK_DURATION {
                trace!(
                    "finished misbehaviour verification after {:?}", start_time.elapsed()
                );
                return Ok(None);
            }
            update = None;
        ...
        trace!("finished misbehaviour verification after {:?}", start_time.elapsed());
        Ok(None)
    }
    #[prusti_contracts::trusted]
    fn submit_evidence(
        ...
        let signer = self
            ...
            .map_err(|e| {
                ForeignClientError::misbehaviour(
                    format!(
                        "failed getting signer for destination chain ({})", self
                        .dst_chain.id()
                    ),
                    e,
                )
        ...
        let events = self
            ...
            .map_err(|e| {
                ForeignClientError::misbehaviour(
                    format!(
                        "failed sending evidence to destination chain ({})", self
                        .dst_chain.id(),
                    ),
                    e,
                )
    ...
    pub fn detect_misbehaviour_and_submit_evidence(
        ...
        let result = match self.detect_misbehaviour(update_event.clone()) {
            ...
            Ok(Some(detected)) => {
                error!(
                    "[{}] MISBEHAVIOUR DETECTED {}, sending evidence", self, detected
                    .misbehaviour
                );
        ...
        match result {
            Err(
                ForeignClientError(ForeignClientErrorDetail::MisbehaviourExit(s), _),
            ) => {
                warn!("[{}] misbehaviour checking is being disabled: {:?}", self, s);
                MisbehaviourResults::CannotExecute
            }
            Ok(misbehaviour_detection_result) => {
                if !misbehaviour_detection_result.is_empty() {
                    info!(
                        "[{}] evidence submission result {:?}", self,
            ...
            Err(e) => {
                match e.detail() {
                    ForeignClientErrorDetail::MisbehaviourExit(s) => {
                        error!(
                            "[{}] misbehaviour checking is being disabled: {:?}", self, s
                        );
                        MisbehaviourResults::CannotExecute
                    }
...
pub fn extract_client_id(event: &IbcEvent) -> Result<&ClientId, ForeignClientError> {
    match event {
        ...
        _ => Err(ForeignClientError::missing_client_id_from_event(event.clone())),
    }
}
...
mod test {
    ...
    fn create_client_and_send_method() {
        ...
        let a_chain = ChainRuntime::<
            MockChain,
        >::spawn::<BaseChainHandle>(a_cfg, rt.clone())
            .unwrap();
        let b_chain = ChainRuntime::<MockChain>::spawn::<BaseChainHandle>(b_cfg, rt)
            .unwrap();
        ...
        assert!(
            res.is_ok(), "build_create_client_and_send failed (chain a) with error {:?}",
            res
        );
        assert!(matches!(res.unwrap(), IbcEvent::CreateClient(_)));
        let res = b_client.build_create_client_and_send(Default::default());
        assert!(
            res.is_ok(), "build_create_client_and_send failed (chain b) with error {:?}",
            res
        );
        assert!(matches!(res.unwrap(), IbcEvent::CreateClient(_)));
    }
    ...
    fn update_client_and_send_method() {
        ...
        let b_chain = ChainRuntime::<MockChain>::spawn::<BaseChainHandle>(b_cfg, rt)
            .unwrap();
        let mut a_client = ForeignClient::restore(
            a_client_id,
            a_chain.clone(),
            b_chain.clone(),
        );
        ...
        let res = a_client.build_latest_update_client_and_send();
        assert!(res.is_err(), "build_update_client_and_send was supposed to fail",);
        ...
        for _i in 1..num_iterations {
            ...
            assert!(
                res.is_ok(),
                "build_update_client_and_send failed (chain a) with error: {:?}", res
            );
            let res = res.unwrap();
            ...
            assert!(
                res.is_ok(),
                "build_update_client_and_send failed (chain b) with error: {:?}", res
            );
            let res = res.unwrap();
            assert!(matches!(res.last(), Some(IbcEvent::UpdateClient(_))));
    ...
    fn foreign_client_create() {
        ...
        assert!(
            ...
            "Client creation (on chain a) failed with error: {:?}", res_client_on_a
        );
        ...
        assert!(
            ...
            "Client creation (on chain a) failed with error: {:?}", res_client_on_b
        );
        let client_on_b = res_client_on_b.unwrap();
        let b_client = client_on_b.id;
        let b_client_state_res = b_chain
            .query_client_state(
                QueryClientStateRequest {
                    client_id: b_client,
                    height: QueryHeight::Latest,
        ...
        assert!(
            ...
            "Client query (on chain b) failed with error: {:?}", b_client_state_res
        );
        let a_client_state_res = a_chain
            .query_client_state(
                QueryClientStateRequest {
                    client_id: a_client,
                    height: QueryHeight::Latest,
        ...
        assert!(
            ...
            "Client query (on chain a) failed with error: {:?}", a_client_state_res
        );
    }
    ...
    fn foreign_client_update() {
        ...
        assert!(
            ...
            "Client creation (on chain a) failed with error: {:?}", client_on_a_res
        );
        ...
        assert!(
            ...
            "Client creation (on chain a) failed with error: {:?}", client_on_b_res
        );
