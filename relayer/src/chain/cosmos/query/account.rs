    ...
    match m_account {
        ...
        None => {
            ...
            *m_account = Some(account.into());
            Ok(m_account.as_mut().expect("account was supposedly just cached"))
        }
    }
}
...
pub async fn refresh_account<'a>(
    ...
    let account = query_account(grpc_address, account_address).await?;
    info!(
        sequence = % account.sequence, number = % account.account_number,
...
pub async fn query_account(
    ...
    if resp_account.type_url == "/cosmos.auth.v1beta1.BaseAccount" {
        ...
        Ok(
            EthAccount::decode(resp_account.value.as_slice())
