...
define_error! {
    Error { SdkModuleNotFound { address : String, app : AppInfo, } | e | {
    format!("failed to find the SDK module dependency ('{}') for application {}", e
    .address, e.app) }, TendermintModuleNotFound { address : String, app : AppInfo, } | e
    | { format!("failed to find the Tendermint dependency ('{}') for application {}", e
    .address, e.app) }, VersionParsingFailed { module_path : String, raw_version :
    String, cause : String, app : AppInfo, } | e | {
    format!("failed parsing the module path ('{}') version number '{}' into a semver for application {}; cause: {}",
...
impl TryFrom<VersionInfo> for Specs {
    ...
    fn try_from(raw_version: VersionInfo) -> Result<Self, Self::Error> {
        ...
        trace!(
            application = % raw_version.app_name, version = % raw_version.version,
            git_commit = % raw_version.git_commit, sdk_version = % sdk_version,
            ibc_go_status = ? ibc_go_version, tendermint_version = % tendermint_version,
...
fn parse_sdk_version(version_info: &VersionInfo) -> Result<semver::Version, Error> {
    let module = version_info
        ...
        .ok_or_else(|| {
            Error::sdk_module_not_found(
                SDK_MODULE_NAME.to_string(),
                AppInfo::from(version_info),
            )
        })?;
    let plain_version = module.version.trim_start_matches('v');
    let mut version = semver::Version::parse(plain_version)
        .map_err(|e| {
...
#[prusti_contracts::trusted]
fn parse_ibc_go_version(
    version_info: &VersionInfo,
) -> Result<Option<semver::Version>, Error> {
    match version_info.build_deps.iter().find(|&m| m.path.contains(IBC_GO_MODULE_NAME)) {
        None => Ok(None),
        Some(ibc_module) => {
