...
pub async fn estimate_tx_fees(
    ...
    debug!("max fee, for use in tx simulation: {}", PrettyFee(& gas_config.max_fee));
...
async fn estimate_fee_with_tx(
    ...
    if estimated_gas > gas_config.max_gas {
        ...
        return Err(
            Error::tx_simulate_gas_estimate_exceeded(
                chain_id.clone(),
                estimated_gas,
                gas_config.max_gas,
            ),
    ...
    debug!(
        id = % chain_id, "send_tx: using {} gas, fee {}", estimated_gas, PrettyFee(&
        adjusted_fee)
    );
    Ok(adjusted_fee)
}
...
async fn estimate_gas_with_tx(
    ...
    let simulated_gas = send_tx_simulate(grpc_address, tx).await.map(|sr| sr.gas_info);
    let _span = span!(Level::ERROR, "estimate_gas").entered();
    match simulated_gas {
        Ok(Some(gas_info)) => {
            debug!("tx simulation successful, gas amount used: {:?}", gas_info.gas_used);
        ...
        Err(e) => {
            error!("failed to simulate tx. propagating error to caller: {}", e.detail());
            Err(e)
        }
    }
}
