...
use ibc::{
    core::{
        ics02_client::{
            ...
            events::UpdateClient, header::{AnyHeader, Header},
...
use crate::{
    account::Balance, config::ChainConfig, connection::ConnectionMsgType,
    denom::DenomTrace, error::Error,
    event::{
        bus::EventBus,
        monitor::{
    ...
    keyring::KeyEntry, light_client::LightClient,
};
use super::{
    client::ClientSettings, endpoint::{ChainEndpoint, ChainStatus, HealthCheck},
...
impl EventMonitorCtrl {
    ...
    pub fn recv(&self) -> &EventReceiver {
        match self {
            ...
            Self::Live { ref event_receiver, .. } => event_receiver,
        }
    }
    #[prusti_contracts::trusted]
    pub fn shutdown(&self) -> Result<(), Error> {
        match self {
            Self::None { .. } => Ok(()),
            Self::Live { ref tx_monitor_cmd, .. } => {
                tx_monitor_cmd.send(MonitorCmd::Shutdown).map_err(Error::send)
            }
        }
    }
    #[prusti_contracts::trusted]
    ...
    Endpoint: ChainEndpoint + Send + 'static,
{
    ...
    fn run(mut self) -> Result<(), Error> {
        loop {
            channel::select! {
                recv(self.event_monitor_ctrl.recv()) -> event_batch => { match
                event_batch { Ok(event_batch) => { self.event_bus
                .broadcast(Arc::new(event_batch)); }, Err(e) => {
                error!("received error via event bus: {}", e); return
                Err(Error::channel_receive(e)); }, } }, recv(self.request_receiver) ->
                event => { match event { Ok(ChainRequest::Shutdown { reply_to }) => {
                self.event_monitor_ctrl.shutdown() ?; let res = self.chain.shutdown();
                reply_to.send(res).map_err(Error::send) ?; break; },
                Ok(ChainRequest::HealthCheck { reply_to }) => { self
                .health_check(reply_to) ? }, Ok(ChainRequest::Subscribe { reply_to }) =>
                ...
                Ok(ChainRequest::Signer { reply_to }) => { self.get_signer(reply_to) ? },
                Ok(ChainRequest::Config { reply_to }) => { self.get_config(reply_to) ? },
                Ok(ChainRequest::GetKey { reply_to }) => { self.get_key(reply_to) ? },
                Ok(ChainRequest::AddKey { key_name, key, reply_to }) => { self
                .add_key(key_name, key, reply_to) ? }, Ok(ChainRequest::IbcVersion {
                reply_to }) => { self.ibc_version(reply_to) ? },
                Ok(ChainRequest::BuildHeader { trusted_height, target_height,
                client_state, reply_to }) => { self.build_header(trusted_height,
                target_height, client_state, reply_to) ? },
                Ok(ChainRequest::BuildClientState { height, settings, reply_to }) => {
                self.build_client_state(height, settings, reply_to) ? },
                Ok(ChainRequest::BuildConsensusState { trusted, target, client_state,
                reply_to }) => { self.build_consensus_state(trusted, target,
                client_state, reply_to) ? }, Ok(ChainRequest::BuildMisbehaviour {
                client_state, update_event, reply_to }) => { self
                .check_misbehaviour(update_event, client_state, reply_to) ? },
                Ok(ChainRequest::BuildConnectionProofsAndClientState { message_type,
                connection_id, client_id, height, reply_to }) => { self
                .build_connection_proofs_and_client_state(message_type, connection_id,
                client_id, height, reply_to) ? }, Ok(ChainRequest::BuildChannelProofs {
                port_id, channel_id, height, reply_to }) => { self
                .build_channel_proofs(port_id, channel_id, height, reply_to) ? },
                Ok(ChainRequest::QueryBalance { key_name, reply_to }) => { self
                .query_balance(key_name, reply_to) ? }, Ok(ChainRequest::QueryDenomTrace
                { hash, reply_to }) => { self.query_denom_trace(hash, reply_to) ? },
                Ok(ChainRequest::QueryApplicationStatus { reply_to }) => { self
                .query_application_status(reply_to) ? }, Ok(ChainRequest::QueryClients {
                request, reply_to }) => { self.query_clients(request, reply_to) ? },
                Ok(ChainRequest::QueryClientConnections { request, reply_to }) => { self
                .query_client_connections(request, reply_to) ? },
                Ok(ChainRequest::QueryClientState { request, include_proof, reply_to })
                => { self.query_client_state(request, include_proof, reply_to) ? },
                Ok(ChainRequest::QueryConsensusStates { request, reply_to }) => { self
                .query_consensus_states(request, reply_to) ? },
                ...
                Ok(ChainRequest::QueryCommitmentPrefix { reply_to }) => { self
                .query_commitment_prefix(reply_to) ? },
                Ok(ChainRequest::QueryCompatibleVersions { reply_to }) => { self
                .query_compatible_versions(reply_to) ? },
                Ok(ChainRequest::QueryConnection { request, include_proof, reply_to }) =>
                { self.query_connection(request, include_proof, reply_to) ? },
                Ok(ChainRequest::QueryConnections { request, reply_to }) => { self
                .query_connections(request, reply_to) ? },
                Ok(ChainRequest::QueryConnectionChannels { request, reply_to }) => { self
                .query_connection_channels(request, reply_to) ? },
                Ok(ChainRequest::QueryChannels { request, reply_to }) => { self
                .query_channels(request, reply_to) ? }, Ok(ChainRequest::QueryChannel {
                request, include_proof, reply_to }) => { self.query_channel(request,
                include_proof, reply_to) ? }, Ok(ChainRequest::QueryChannelClientState {
                request, reply_to }) => { self.query_channel_client_state(request,
                reply_to) ? }, Ok(ChainRequest::BuildPacketProofs { packet_type, port_id,
                channel_id, sequence, height, reply_to }) => { self
                .build_packet_proofs(packet_type, port_id, channel_id, sequence, height,
                reply_to) ? }, Ok(ChainRequest::QueryPacketCommitment { request,
                include_proof, reply_to }) => { self.query_packet_commitment(request,
                include_proof, reply_to) ? }, Ok(ChainRequest::QueryPacketCommitments {
                request, reply_to }) => { self.query_packet_commitments(request,
                reply_to) ? }, Ok(ChainRequest::QueryPacketReceipt { request,
                include_proof, reply_to }) => { self.query_packet_receipt(request,
                include_proof, reply_to) ? }, Ok(ChainRequest::QueryUnreceivedPackets {
                request, reply_to }) => { self.query_unreceived_packets(request,
                reply_to) ? }, Ok(ChainRequest::QueryPacketAcknowledgement { request,
                include_proof, reply_to }) => { self
                .query_packet_acknowledgement(request, include_proof, reply_to) ? },
                ...
                Ok(ChainRequest::QueryNextSequenceReceive { request, include_proof,
                reply_to }) => { self.query_next_sequence_receive(request, include_proof,
                reply_to) ? }, Ok(ChainRequest::QueryPacketEventDataFromTxs { request,
                reply_to }) => { self.query_txs(request, reply_to) ? },
                Ok(ChainRequest::QueryPacketEventDataFromBlocks { request, reply_to }) =>
                { self.query_blocks(request, reply_to) ? },
                Ok(ChainRequest::QueryHostConsensusState { request, reply_to }) => { self
                .query_host_consensus_state(request, reply_to) ? }, Err(e) =>
    ...
    fn enable_event_monitor(&mut self) -> Result<(), Error> {
        let (event_receiver, tx_monitor_cmd) = self
            .chain
            .init_event_monitor(self.rt.clone())?;
        self.event_monitor_ctrl.enable(event_receiver, tx_monitor_cmd);
        Ok(())
    }
    #[prusti_contracts::trusted]
    ...
    fn build_connection_proofs_and_client_state(
        ...
        let result = result
            .map(|(opt_client_state, proofs)| (
                opt_client_state.map(|cs| cs.wrap_any()),
                proofs,
            ));
        reply_to.send(result).map_err(Error::send)
    }
    ...
    fn build_channel_proofs(
        ...
        let result = self.chain.build_channel_proofs(&port_id, &channel_id, height);
        reply_to.send(result).map_err(Error::send)
    }
    #[prusti_contracts::trusted]
    ...
    fn query_packet_acknowledgement(
        ...
        let result = self.chain.query_packet_acknowledgement(request, include_proof);
        reply_to.send(result).map_err(Error::send)
    }
    #[prusti_contracts::trusted]
    ...
    fn query_next_sequence_receive(
        ...
        let result = self.chain.query_next_sequence_receive(request, include_proof);
        reply_to.send(result).map_err(Error::send)
    }
    #[prusti_contracts::trusted]
    fn query_txs(
