...
use bitcoin::{
    network::constants::Network, secp256k1::{Message, Secp256k1, SecretKey},
...
impl KeyEntry {
    ...
    pub fn from_key_file(key_file: KeyFile, hd_path: &HDPath) -> Result<Self, Error> {
        ...
        let keyfile_pubkey_bytes = keyfile_pubkey_bytes
            .split_off(keyfile_pubkey_bytes.len() - derived_pubkey_bytes.len());
        if keyfile_pubkey_bytes != derived_pubkey_bytes {
            Err(Error::public_key_mismatch(keyfile_pubkey_bytes, derived_pubkey_bytes))
        } else {
            Ok(Self {
...
impl KeyStore for Memory {
    #[prusti_contracts::trusted]
    fn get_key(&self, key_name: &str) -> Result<KeyEntry, Error> {
        self.keys.get(key_name).cloned().ok_or_else(Error::key_not_found)
    }
    #[prusti_contracts::trusted]
    fn add_key(&mut self, key_name: &str, key_entry: KeyEntry) -> Result<(), Error> {
    ...
    #[prusti_contracts::trusted]
    fn remove_key(&mut self, key_name: &str) -> Result<(), Error> {
        self.keys.remove(key_name).ok_or_else(Error::key_not_found)?;
        Ok(())
    }
    #[prusti_contracts::trusted]
    fn keys(&self) -> Result<Vec<(String, KeyEntry)>, Error> {
        Ok(self.keys.iter().map(|(n, k)| (n.to_string(), k.clone())).collect())
    }
}
#[derive(Clone, Debug, Serialize, Deserialize)]
pub struct Test {
    ...
    pub fn new(
        ...
        match store {
            ...
            Store::Test => {
                ...
                Ok(Self::Test(Test::new(account_prefix.to_string(), keys_folder)))
            }
        }
    }
    #[prusti_contracts::trusted]
...
pub fn sign_message(
    ...
    match address_type {
        AddressType::Ethermint {
            ref pk_type,
        } if pk_type.ends_with(".ethsecp256k1.PubKey") => {
...
fn standard_path_to_derivation_path(path: &StandardHDPath) -> DerivationPath {
    ...
    let child_numbers = vec![
        ...
        .expect("Purpose is not Hardened"), ChildNumber::from_hardened_idx(path
        .coin_type()).expect("Coin Type is not Hardened"),
