...
use crate::{
    ...
    object::Connection, util::retry::retry_with_index, worker::retry_strategy,
};
        ...
        move || {
            if let Ok(cmd) = cmd_rx.try_recv() {
                match cmd {
                    ...
                    WorkerCmd::NewBlock { height: current_height, new_block: _ } => {
                        ...
                        retry_with_index(
                                retry_strategy::worker_default_strategy(),
                                |index| { handshake_connection.step_state(state, index) },
                            )
                            .map_err(|e| TaskError::Fatal(RunError::retry(e)))
                    }
