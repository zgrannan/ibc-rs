...
pub fn spawn_worker_tasks<ChainA: ChainHandle, ChainB: ChainHandle>(
    ...
    let (cmd_tx, data) = match &object {
        ...
        Object::Connection(connection) => {
            let (cmd_tx, cmd_rx) = crossbeam_channel::unbounded();
            let connection_task = connection::spawn_connection_worker(
                connection.clone(),
                chains,
                cmd_rx,
            );
        ...
        Object::Channel(channel) => {
            let (cmd_tx, cmd_rx) = crossbeam_channel::unbounded();
            let channel_task = channel::spawn_channel_worker(
                channel.clone(),
                chains,
                cmd_rx,
            );
        ...
        Object::Packet(path) => {
            ...
            match link_res {
                Ok(link) => {
                    ...
                    let should_clear_on_start = packets_config.clear_on_start
                        || channel_ordering == Order::Ordered;
                    let (cmd_tx, cmd_rx) = crossbeam_channel::unbounded();
                    let link = Arc::new(Mutex::new(link));
                    let resubmit = Resubmit::from_clear_interval(
