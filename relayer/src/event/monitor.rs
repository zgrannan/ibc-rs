...
use futures::{
    pin_mut, stream::{self, select_all, StreamExt},
    Stream, TryStreamExt,
};
...
use crate::{
    chain::tracking::TrackingId, telemetry,
    util::{
...
pub mod queries {
    ...
    pub fn all() -> Vec<Query> {
        vec![new_block(), ibc_client(), ibc_connection(), ibc_channel(),]
    }
    #[prusti_contracts::trusted]
...
impl EventMonitor {
    ...
    pub fn new(
        ...
        let (client, driver) = rt
            ...
            .map_err(|_| Error::client_creation_failed(
                chain_id.clone(),
                node_addr.clone(),
            ))?;
    ...
    fn try_reconnect(&mut self) -> Result<()> {
        trace!(
            "[{}] trying to reconnect to WebSocket endpoint {}", self.chain_id, self
            .node_addr
        );
        ...
        trace!(
            "[{}] reconnected to WebSocket endpoint {}", self.chain_id, self.node_addr
        );
        trace!("[{}] gracefully shutting down previous client", self.chain_id);
        let _ = client.close();
        self.rt.block_on(driver_handle).map_err(Error::client_termination_failed)?;
    ...
    fn reconnect(&mut self) {
        ...
        match result {
            Ok(()) => {
                info!(
                    "[{}] successfully reconnected to WebSocket endpoint {}", self
                    .chain_id, self.node_addr
                )
            }
            Err(retries) => {
                error!(
                    "[{}] failed to reconnect to {} after {} retries", self.chain_id,
    ...
    pub fn run(mut self) {
        ...
        trace!("[{}] event monitor has successfully shut down", self.chain_id);
    }
    #[prusti_contracts::trusted]
    fn run_loop(&mut self) -> Next {
        let subscriptions = core::mem::replace(
        ...
        loop {
            ...
            let result = rt
                .block_on(async {
                    tokio::select! {
                        Some(batch) = batches.next() => batch, Some(e) = self.rx_err
                        .recv() => Err(Error::web_socket_driver(e)),
                    }
                });
            if let Ok(MonitorCmd::Shutdown) = self.rx_cmd.try_recv() {
                return Next::Abort;
            ...
            match result {
                ...
                Err(e) => {
                    if let ErrorDetail::SubscriptionCancelled(reason) = e.detail() {
                        error!(
                            "[{}] subscription cancelled, reason: {}", self.chain_id,
                            reason
                        );
    ...
    fn propagate_error(&self, error: Error) -> Result<()> {
        self.tx_batch.send(Err(error)).map_err(|_| Error::channel_send_failed())?;
        Ok(())
    }
    ...
    fn process_batch(&self, batch: EventBatch) -> Result<()> {
        ...
        self.tx_batch.send(Ok(batch)).map_err(|_| Error::channel_send_failed())?;
        Ok(())
    }
}
    ...
    grouped
        .map_ok(move |events| {
            let height = events
                ...
                .map(|(h, _)| h)
                .copied()
                .expect("internal error: found empty group");
