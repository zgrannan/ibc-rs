...
use crate::{
    chain::{
        ...
        handle::ChainHandle,
        requests::{
            IncludeProof, PageRequest, QueryChannelRequest,
            QueryClientConnectionsRequest, QueryClientStateRequest,
            QueryClientStatesRequest, QueryConnectionChannelsRequest,
            QueryConnectionRequest, QueryHeight,
        },
    },
    config::{filter::ChannelFilters, ChainConfig, Config, PacketFilter},
    registry::Registry, supervisor::client_state_filter::{FilterPolicy, Permission},
};
use crate::chain::counterparty::{unreceived_acknowledgements, unreceived_packets};
use crate::error::Error as RelayerError;
use crate::spawn::SpawnError;
flex_error::define_error! {
    Error { Spawn[SpawnError] | _ | { "spawn" }, Query[RelayerError] | _ | { "query" },
    MissingConnectionHop { port_id : PortId, channel_id : ChannelId, chain_id : ChainId,
    } | e | {
    format_args!("could not retrieve the connection hop underlying port/channel {}/{} on chain '{}'",
    e.port_id, e.channel_id, e.chain_id) }, UninitializedChannel { port_id : PortId,
    channel_id : ChannelId, chain_id : ChainId, } | e | {
    format_args!("channel '{}/{}' on chain '{}' is uninitialized", e.port_id, e
    .channel_id, e.chain_id) }, CounterpartyConnectionState { connection_id :
    ConnectionId, counterparty_chain_id : ChainId, reason : String, } | e | {
    format_args!("failed to query counterparty connection state of connection '{}' on counterparty chain '{}', reason: {}",
            ...
            for client in scan.clients.values() {
                ...
                for conn in client.connections.values() {
                    ...
                    for chan in conn.channels.values() {
                        ...
                        writeln!(
                            f, "        | State: {}", chan.channel.channel_end.state()
                        )?;
...
impl<'a, Chain: ChainHandle> ChainScanner<'a, Chain> {
            ...
            match result {
                Ok(
                    ...
                    connection_scan
                        ...
                        .or_insert_with(|| ChannelScan::new(
                            channel,
                            counterparty_channel,
                        ));
                }
                Err(e) => {
                    error!(
                        channel = % channel_id, "failed to scan channel, reason: {}", e
                    )
                }
            }
        }
    ...
    fn scan_client(
        ...
        if !has_counterparty {
            debug!(
                chain = % chain.id(), counterparty_chain = % counterparty_chain_id,
        ...
        for connection_end in client_connections_ids {
            ...
            {
                scan.connections.insert(connection_scan.id().clone(), connection_scan);
            }
        }
        Ok(Some(scan))
    }
    ...
    fn scan_connection(
        ...
        let span = info_span!(
            "scan.connection", connection = % connection.connection_id
        );
        let _guard = span.enter();
        ...
        if !scan.is_open() {
            warn!(
                "connection is not open, skipping scan of channels over this connection"
            );
            return Ok(Some(scan));
        }
        let counterparty_state = match self
            .counterparty_connection_state(client, &scan.connection)
        {
            Ok(state) if !state.eq(&ConnectionState::Open) => {
                warn!(
                    "counterparty connection is not open, skipping scan of channels over this connection"
                );
                return Ok(Some(scan));
            }
            Err(e) => {
        ...
        let channels = channels
            ...
            .map(|channel| {
                let counterparty = channel_on_destination(
                        &channel,
                        &scan.connection,
                        &counterparty_chain,
                    )
    ...
    fn counterparty_connection_state(
        ...
        let counterparty_chain = self
            ...
            .get_or_spawn(&client.client_state.chain_id())
            .map_err(Error::spawn)?;
        let counterparty_state = connection_state_on_destination(
                connection,
                &counterparty_chain,
            )
            .map_err(|e| {
    ...
    fn client_allowed(
        ...
        let permission = self
            .client_state_filter
            .control_client(&chain.id(), &client.client_id, &client.client_state);
        ...
        match permission {
            Ok(Permission::Deny) => {
                warn!(
                    ...
                    chain.id(), client.client_id, connection.connection_id
                );
                false
            }
            Err(e) => {
                error!(
                    ...
                    chain.id(), client.client_id, connection.connection_id, e
                );
                false
            }
            _ => true,
...
fn scan_allowed_channel<Chain: ChainHandle>(
    ...
    if channel.channel_end.state_matches(&ChannelState::Uninitialized) {
        return Err(
            Error::uninitialized_channel(port_id.clone(), channel_id.clone(), chain.id()),
        );
    }
    ...
    info!(
        client = % client_id, counterparty_chain = % client.client_state.chain_id(),
    ...
    let counterparty_channel_name = counterparty_channel
        ...
        .unwrap_or_else(|| "<none>".to_string());
    info!(
        counterparty_channel = % counterparty_channel_name, "found counterparty channel"
    );
    let counterparty_connection_state = connection_state_on_destination(
            &connection,
            &counterparty_chain,
        )
        .map(Some)
...
fn query_channel<Chain: ChainHandle>(
    ...
    let (channel_end, _) = chain
        ...
        .map_err(Error::query)?;
    Ok(IdentifiedChannelEnd::new(port_id.clone(), channel_id.clone(), channel_end))
}
#[prusti_contracts::trusted]
