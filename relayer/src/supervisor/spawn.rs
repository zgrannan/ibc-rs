...
use crate::{
    ...
    config::Config, object::{Channel, Client, Connection, Object, Packet, Wallet},
    registry::Registry, supervisor::error::Error as SupervisorError, telemetry,
    worker::WorkerMap,
};
use super::{
...
impl<'a, Chain: ChainHandle> SpawnContext<'a, Chain> {
    ...
    pub fn spawn_workers(&mut self, scan: ChainsScan) {
        for chain_scan in scan.chains {
            match chain_scan {
                ...
                Err(e) => error!("failed to spawn worker for a chain, reason: {}", e),
            }
        }
    }
    #[prusti_contracts::trusted]
    ...
    pub fn spawn_wallet_worker(&mut self, chain: Chain) {
        let wallet_object = Object::Wallet(Wallet { chain_id: chain.id() });
        self.workers
    ...
    pub fn spawn_workers_for_connection(
            ...
            )
        {
            Ok(true) => {
                info!(
                    chain = % chain.id(), connection = % connection_id,
            ...
            Err(e) => {
                error!(
                    chain = % chain.id(), connection = % connection_id,
                    "skipped connection workers, reason: {}", e
                )
            }
        }
        for (channel_id, channel_scan) in connection_scan.channels {
            match self.spawn_workers_for_channel(chain.clone(), client, channel_scan) {
                Ok(true) => {
                    info!(
                        chain = % chain.id(), channel = % channel_id,
                ...
                Err(e) => {
                    error!(
                        chain = % chain.id(), channel = % channel_id,
                        "skipped channel workers, reason: {}", e
                    )
                }
            }
        }
    ...
    fn spawn_connection_workers(
        ...
        let conn_state_dst = connection_state_on_destination(
            &connection,
            &counterparty_chain,
        )?;
        info!(
            chain = % chain.id(), connection = % connection.connection_id,
            counterparty_chain = % counterparty_chain.id(),
            "connection is {:?}, state on destination chain is {:?}", conn_state_src,
            conn_state_dst
        );
        if conn_state_src.is_open() && conn_state_dst.is_open() {
            ...
            Ok(false)
        } else if config_conn_enabled && !conn_state_dst.is_open()
            ...
            self.workers
                ...
                .then(|| {
                    info!(
                        "spawning Connection worker: {}", connection_object.short_name()
                    );
                });
            Ok(true)
        } else {
    ...
    pub fn spawn_workers_for_channel(
        ...
        info!(
            chain = % chain.id(), counterparty_chain = % counterparty_chain.id(), channel
            = % channel_scan.id(), "channel is {}, state on destination chain is {}",
            chan_state_src, chan_state_dst
        );
        if (mode.clients.enabled || mode.packets.enabled) && chan_state_src.is_open()
            && chan_state_dst.is_open()
        {
            if mode.clients.enabled {
                ...
                self.workers
                    ...
                    .then(|| {
                        info!("spawned client worker: {}", client_object.short_name())
                    });
            }
            if mode.packets.enabled {
                ...
                let has_acks = || {
                    !channel_scan
                        .unreceived_acknowledgements_on_counterparty(
                            &chain,
                            &counterparty_chain,
                        )
                        .unwrap_or_default()
                ...
                if has_packets() || has_acks() {
                    ...
                    self.workers
                        ...
                        .then(|| {
                            info!("spawned packet worker: {}", path_object.short_name())
                        });
                }
            }
            Ok(mode.clients.enabled)
        } else if mode.channels.enabled && !chan_state_dst.is_open()
            ...
            self.workers
                ...
                .then(|| {
                    info!("spawned channel worker: {}", channel_object.short_name())
                });
            Ok(true)
        } else {
            Ok(false)
