...
use tendermint_light_client::{
    ...
    light_client::LightClient as TmLightClient, state::State as LightClientState,
...
use tendermint_light_client_verifier::types::{
    Height as TMHeight, LightBlock, PeerId, Status,
};
...
impl super::LightClient<CosmosSdkChain> for LightClient {
    ...
    fn header_and_minimal_set(
        ...
        let Verified { target, supporting } = self
    ...
    fn verify(
        ...
        let target = client
            ...
            .map_err(|e| Error::light_client_verification(
    ...
    fn check_misbehaviour(
        ...
        let update_header = update
            .header
            .clone()
            .ok_or_else(|| {
                Error::misbehaviour(
                    format!("missing header in update client event {}", self.chain_id),
                )
            })?;
        let update_header = downcast!(update_header => AnyHeader::Tendermint)
            .ok_or_else(|| {
                Error::misbehaviour(
                    format!("header type incompatible for chain {}", self.chain_id),
                )
            })?;
        ...
        if !headers_compatible(&target.signed_header, &update_header.signed_header) {
            let (witness, supporting) = self
                .adjust_headers(trusted_height, target, supporting)?;
            let misbehaviour = TmMisbehaviour {
                ...
                .wrap_any();
            Ok(
                Some(MisbehaviourEvidence {
                    misbehaviour,
                    supporting_headers: supporting
                        .into_iter()
                        .map(TmHeader::wrap_any)
                        .collect(),
                }),
            )
        } else {
...
impl LightClient {
    ...
    pub fn from_config(config: &ChainConfig, peer_id: PeerId) -> Result<Self, Error> {
        ...
        let io = components::io::ProdIo::new(
            peer_id,
            rpc_client,
