...
pub fn retry_count<E>(err: &RetryError<E>) -> u64 {
    match err {
        RetryError::Operation { tries, total_delay: _, error: _ } => *tries,
        _ => 0,
    }
}
...
pub fn clamp(
    ...
    strategy.take(max_retries).map(move |delay| delay.min(max_delay))
}
#[prusti_contracts::trusted]
pub fn clamp_total(
    strategy: impl Iterator<Item = Duration>,
...
mod tests {
    use super::*;
    use test_log::test;
    const CONST_STRATEGY: ConstantGrowth = ConstantGrowth::new(
    ...
    fn const_growth_no_clamp() {
        ...
        assert_eq!(
            delays, vec![Duration::from_millis(1000), Duration::from_millis(1500),
            Duration::from_millis(2000), Duration::from_millis(2500),
            Duration::from_millis(3000), Duration::from_millis(3500),
            Duration::from_millis(4000), Duration::from_millis(4500),
            Duration::from_millis(5000), Duration::from_millis(5500)]
        );
    }
    #[test]
    #[prusti_contracts::trusted]
    ...
    fn clamped_total_const_growth_max_retries() {
        ...
        let delays = strategy.collect::<Vec<_>>();
        assert_eq!(
            delays, vec![Duration::from_millis(200), Duration::from_millis(300),
