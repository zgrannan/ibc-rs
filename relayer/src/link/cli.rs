...
impl<ChainA: ChainHandle, ChainB: ChainHandle> Link<ChainA, ChainB> {
    ...
    pub fn relay_recv_packet_and_timeout_messages(
        ...
        let _span = error_span!(
            "PacketRecvCmd", src_chain = % self.a_to_b.src_chain().id(), src_port = %
            self.a_to_b.src_port_id(), src_channel = % self.a_to_b.src_channel_id(),
    ...
    pub fn relay_ack_packet_messages(&self) -> Result<Vec<IbcEvent>, LinkError> {
        let _span = error_span!(
            "PacketAckCmd", src_chain = % self.a_to_b.src_chain().id(), src_port = % self
            .a_to_b.src_port_id(), src_channel = % self.a_to_b.src_channel_id(),
            dst_chain = % self.a_to_b.dst_chain().id(),
        )
            .entered();
    ...
    fn relay_packet_messages(
        ...
        for events_chunk in query_packet_events_with(
            ...
            self.a_to_b.relay_and_accumulate_results(Vec::from(src_ods), &mut results)?;
            self.a_to_b.relay_and_accumulate_results(Vec::from(dst_ods), &mut results)?;
        }
        while let Some(odata) = self.a_to_b.fetch_scheduled_operational_data()? {
            self.a_to_b.relay_and_accumulate_results(vec![odata], &mut results)?;
        }
        Ok(results)
    }
}
    ...
    LatestHeight: Fn() -> Result<Height, LinkError>,
{
    let (time_left, blocks_left) = odata
        .conn_delay_remaining(chain_time, max_expected_time_per_block, latest_height)?;
    match (time_left, blocks_left) {
        (Duration::ZERO, 0) => {
            info!(
                "ready to fetch a scheduled op. data with batch of size {} targeting {}",
                odata.batch.len(), odata.target,
            );
            Ok(odata)
        }
        (Duration::ZERO, blocks_left) => {
            info!(
                ...
                blocks_left, odata.batch.len(), odata.target,
            );
            let blocks_left: u32 = blocks_left
                .try_into()
                .expect("blocks_left > u32::MAX");
        ...
        (time_left, _) => {
            info!(
                ...
                time_left, odata.batch.len(), odata.target,
            );
