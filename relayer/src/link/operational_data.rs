...
impl OperationalData {
    ...
    pub fn assemble_msgs<ChainA: ChainHandle, ChainB: ChainHandle>(
        ...
        let client_update_msg = if !self.conn_delay_needed() {
            ...
            debug!(
                "prepending {} client update at height {}", self.target, update_height
            );
    ...
    fn get_delay_if_needed(&self) -> Option<&ConnectionDelay> {
        self.connection_delay.as_ref().filter(|_| self.has_packet_msgs())
    }
    ...
    {
        Ok(
            self.conn_time_delay_remaining(chain_time)?.is_zero()
                && self
                    .conn_block_delay_remaining(
...
impl ConnectionDelay {
    ...
    fn conn_time_delay_remaining(&self, chain_time: Instant) -> Duration {
        ...
        if elapsed >= self.delay { Duration::ZERO } else { self.delay - elapsed }
    }
    ...
    fn conn_block_delay_remaining(
        ...
        if latest_height >= acceptable_height {
            ...
            debug_assert!(
                acceptable_height.revision_number() == latest_height.revision_number()
            );
...
impl fmt::Display for OperationalInfo {
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            f, "{} ->{} @{}; len={}", self.tracking_id, self.target, self.proofs_height,
            self.batch_len,
        )
    }
}
