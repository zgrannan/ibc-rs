...
use ibc::{
    core::{
        ...
        ics04_channel::{
            ...
            msgs::{
                ...
                chan_close_confirm::MsgChannelCloseConfirm, recv_packet::MsgRecvPacket,
    ...
    signer::Signer, timestamp::Timestamp, tx_msg::Msg, Height,
};
...
impl Resubmit {
    ...
    pub fn from_clear_interval(clear_interval: u64) -> Self {
        if clear_interval == 0 { Self::Yes } else { Self::No }
    }
}
...
impl<ChainA: ChainHandle, ChainB: ChainHandle> RelayPath<ChainA, ChainB> {
    ...
    pub fn new(
        ...
        Ok(Self {
            ...
            pending_txs_src: PendingTxs::new(
                src_chain,
                src_channel_id,
                src_port_id,
                dst_chain_id,
            ),
            pending_txs_dst: PendingTxs::new(
                dst_chain,
                dst_channel_id,
                dst_port_id,
                src_chain_id,
    ...
    fn src_channel(&self, height_query: QueryHeight) -> Result<ChannelEnd, LinkError> {
        self.src_chain()
            ...
            .map_err(|e| LinkError::channel(
                ChannelError::query(self.src_chain().id(), e),
            ))
    }
    #[prusti_contracts::trusted]
    fn dst_channel(&self, height_query: QueryHeight) -> Result<ChannelEnd, LinkError> {
        self.dst_chain()
            ...
            .map(|(channel_end, _)| channel_end)
            .map_err(|e| LinkError::channel(
                ChannelError::query(self.dst_chain().id(), e),
            ))
    }
    #[prusti_contracts::trusted]
    ...
    pub(crate) fn src_time_latest(&self) -> Result<Instant, LinkError> {
        let elapsed = Timestamp::now()
            .duration_since(
                &self.src_chain().query_application_status().unwrap().timestamp,
            )
            .unwrap_or_default();
    ...
    pub(crate) fn dst_time_latest(&self) -> Result<Instant, LinkError> {
        let elapsed = Timestamp::now()
            .duration_since(
                &self.dst_chain().query_application_status().unwrap().timestamp,
            )
            .unwrap_or_default();
        Ok(Instant::now().sub(elapsed))
    }
    ...
    pub(crate) fn src_max_block_time(&self) -> Result<Duration, LinkError> {
        Ok(self.src_chain().config().map_err(LinkError::relayer)?.max_block_time)
    }
    #[prusti_contracts::trusted]
    pub(crate) fn dst_max_block_time(&self) -> Result<Duration, LinkError> {
        Ok(self.dst_chain().config().map_err(LinkError::relayer)?.max_block_time)
    }
    #[prusti_contracts::trusted]
    ...
    pub fn build_update_client_on_dst(
        ...
        client.wait_and_build_update_client(height).map_err(LinkError::client)
    }
    #[prusti_contracts::trusted]
    ...
    fn relay_pending_packets(&self, height: Option<Height>) -> Result<(), LinkError> {
        ...
        for i in 1..=MAX_RETRIES {
            ...
            match cleared {
                Ok(()) => return Ok(()),
                Err(e) => {
                    error!("failed to clear packets, retry {}/{}: {}", i, MAX_RETRIES, e)
                }
            }
        }
    ...
    pub fn update_schedule(&self, batch: EventBatch) -> Result<(), LinkError> {
        ...
        telemetry!(
            { for e in events.events() { self
            .record_send_packet_and_acknowledgment_history(e); } }
        );
    ...
    fn generate_operational_data(
        ...
        let src_od_res = if src_od.batch.is_empty() { None } else { Some(src_od) };
        let dst_od_res = if dst_od.batch.is_empty() { None } else { Some(dst_od) };
        ...
        for i in 0..MAX_RETRIES {
            ...
            match self.send_from_operational_data::<S>(&odata) {
                Ok(reply) => {
                    info!("success");
                    telemetry!(
                        { let (chain, counterparty, channel_id, port_id) = self
                        .target_info(odata.target); ibc_telemetry::global()
                        .tx_submitted(reply.len(), odata.tracking_id, & chain,
                        channel_id, port_id, & counterparty,); }
                    );
                    return Ok(reply);
                }
    ...
    pub(crate) fn regenerate_operational_data(
        ...
        warn!("failed. Regenerate operational data from {} events", op_info.batch_len());
        let (src_opt, dst_opt) = match self
            .generate_operational_data(initial_odata.into_events())
        {
    ...
    fn send_packet_event_handled(&self, sp: &SendPacket) -> Result<bool, LinkError> {
        Ok(
            self.send_packet_received_on_dst(&sp.packet)?
    ...
    ) -> (Option<IbcEvent>, Option<UpdateClientEvent>, Option<ClientMisbehaviourEvent>) {
        let mut error = None;
        let mut update = None;
    ...
    fn chain_time_at_height(
        ...
        let chain_time = chain
            ...
            .map_err(LinkError::relayer)?
            .timestamp();
        let duration = Timestamp::now().duration_since(&chain_time).unwrap_or_default();
    ...
    fn do_update_client_dst(
        ...
        info!(
            "sending update_client to client hosted on source chain for height {} (retries left: {})",
            src_chain_height, retries_left
        );
    ...
    pub fn schedule_recv_packet_and_timeout_msgs(
        ...
        let _span = span!(
            Level::DEBUG, "schedule_recv_packet_and_timeout_msgs", query_height = ?
            opt_query_height
        )
            .entered();
        ...
        debug!(
            ...
            self.dst_chain().id(), self.src_chain().id(), sequences.iter().take(10)
            .format(", "), sequences.len()
        );
        for events_chunk in query_packet_events_with(
            ...
            telemetry!(
                { for e in events_chunk.clone() { self
                .record_cleared_send_packet_and_acknowledgment(e); } }
            );
            self.events_to_operational_data(
                TrackedEvents::new(events_chunk, tracking_id),
            )?;
        }
        Ok(())
    }
    ...
    pub fn schedule_packet_ack_msgs(
        ...
        let _span = span!(Level::DEBUG, "build_packet_ack_msgs", h = ? opt_query_height)
            .entered();
    ...
    fn build_recv_packet(
        ...
        let proofs = self
            ...
            .map_err(|e| LinkError::packet_proofs_constructor(
                self.src_chain().id(),
                e,
            ))?;
        let msg = MsgRecvPacket::new(packet.clone(), proofs.clone(), self.dst_signer()?);
        trace!(
            "built recv_packet msg {}, proofs at height {}", msg.packet, proofs.height()
        );
        Ok(Some(msg.to_any()))
    }
    #[prusti_contracts::trusted]
    fn build_ack_from_recv_event(
        ...
        trace!(
            "built acknowledgment msg {}, proofs at height {}", msg.packet, proofs
            .height()
        );
        Ok(Some(msg.to_any()))
    }
    ...
    fn build_timeout_packet(
        ...
        trace!("built timeout msg {}, proofs at height {}", msg.packet, proofs.height());
        Ok(Some(msg.to_any()))
    }
    #[prusti_contracts::trusted]
    fn build_timeout_on_close_packet(
        ...
        trace!(
            "built timeout on close msg {}, proofs at height {}", msg.packet, proofs
            .height()
        );
        Ok(Some(msg.to_any()))
    }
    ...
    pub fn execute_schedule(&mut self) -> Result<(), LinkError> {
        ...
        match self
            .execute_schedule_for_target_chain(
                src_od_iter,
                OperationalDataTarget::Source,
            )
        {
    ...
    pub fn process_pending_txs(&self, resubmit: Resubmit) -> RelaySummary {
        ...
        let mut summary_src = self
            .process_pending_txs_src(resubmit)
            .unwrap_or_else(|e| {
                error!("error processing pending events in source chain: {}", e);
                RelaySummary::empty()
            });
        let summary_dst = self
            .process_pending_txs_dst(resubmit)
            .unwrap_or_else(|e| {
                error!("error processing pending events in destination chain: {}", e);
                RelaySummary::empty()
            });
    ...
    pub fn refresh_schedule(&self) -> Result<(), LinkError> {
                ...
                match event {
                    IbcEvent::SendPacket(e) => {
                        if self.send_packet_event_handled(e)? {
                            ...
                            {
                            debug!(
                                "found a timed-out msg in the op data {}", odata.info(),
                            );
                            timed_out
                                .entry(odata_pos)
                                .or_insert_with(|| {
        ...
        for (_, new_od) in timed_out.into_iter() {
            info!(
                "re-scheduling from new timed-out batch of size {}", new_od.batch.len()
            );
    ...
    fn schedule_operational_data(
        ...
        let scheduled_time = if od.conn_delay_needed() {
            ...
            match od.target {
                OperationalDataTarget::Source => {
                    let update_height = self
                        .update_client_src(target_height, od.tracking_id)?;
                    od.set_update_height(update_height);
                    self.src_time_at_height(update_height)?
                }
                OperationalDataTarget::Destination => {
                    let update_height = self
                        .update_client_dst(target_height, od.tracking_id)?;
                    od.set_update_height(update_height);
    ...
    pub(crate) fn try_fetch_scheduled_operational_data(
        ...
        let (elapsed_src_ods, unelapsed_src_ods) = partition(
            self.src_operational_data.take(),
            |op| {
        ...
        let (elapsed_dst_ods, unelapsed_dst_ods) = partition(
            self.dst_operational_data.take(),
            |op| {
                op.has_conn_delay_elapsed(
                    &|| self.dst_time_latest(),
                    &|| self.dst_max_block_time(),
