...
pub fn query_packet_events_with<'a, ChainA>(
    ...
    query_fn: impl Fn(
        ...
        Vec<Sequence>,
        Height,
    ) -> Result<Vec<IbcEvent>, LinkError> + 'a,
        ...
        .map_while(move |c| {
            ...
            match query_fn(src_chain, path, sequences_nrs_chunk, query_height) {
                Ok(mut events) => {
                    ...
                    info!(
                        events_total = % events_total_count, events_left = %
                        events_left_count, "pulled packet data for {} events;", events
                        .len()
                    );
...
pub fn query_send_packet_events<ChainA: ChainHandle>(
    ...
    let _span = span!(Level::DEBUG, "query_send_packet_events", h = % src_query_height)
        .entered();
