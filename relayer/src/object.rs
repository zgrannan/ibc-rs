...
use crate::chain::{
    ...
    handle::ChainHandle, requests::{IncludeProof, QueryClientStateRequest, QueryHeight},
};
...
impl Client {
    ...
    pub fn short_name(&self) -> String {
        format!(
            "client::{}->{}:{}", self.src_chain_id, self.dst_chain_id, self.dst_client_id
        )
    }
}
/// Connection
...
impl Connection {
    ...
    pub fn short_name(&self) -> String {
        format!(
            "connection::{}:{}->{}", self.src_connection_id, self.src_chain_id, self
            .dst_chain_id,
        )
    }
}
...
impl Channel {
    ...
    pub fn short_name(&self) -> String {
        format!(
            "channel::{}/{}:{}->{}", self.src_channel_id, self.src_port_id, self
            .src_chain_id, self.dst_chain_id,
        )
    }
}
...
impl Packet {
    ...
    pub fn short_name(&self) -> String {
        format!(
            "packet::{}/{}:{}->{}", self.src_channel_id, self.src_port_id, self
            .src_chain_id, self.dst_chain_id,
        )
    }
}
...
define_error! {
    ObjectError { Relayer[RelayerError] | _ | { "relayer error" },
    Supervisor[SupervisorError] | _ | { "supervisor error" }, RefreshNotRequired {
    client_id : ClientId, chain_id : ChainId, } | e | {
    format!("client '{}' on chain {} does not require refresh", e.client_id, e.chain_id)
    }, MissingChannelId { event : Attributes } | e | {
    format!("channel_id missing in channel open event '{:?}'", e.event) },
    MissingConnectionId { event : ConnectionAttributes } | e | {
    format!("connection_id missing from connection handshake event '{:?}'", e.event) }, }
}
impl Object {
...
impl Object {
    ...
    pub fn for_update_client(
        ...
        if client_state.refresh_period().is_none() {
            return Err(
                ObjectError::refresh_not_required(e.client_id().clone(), dst_chain.id()),
            );
        }
    ...
    pub fn client_from_chan_open_events(
        ...
        if client.client_state.refresh_period().is_none() {
            return Err(ObjectError::refresh_not_required(client.client_id, chain.id()));
        }
        Ok(
            Client {
