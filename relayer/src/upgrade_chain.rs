...
define_error! {
    UpgradeChainError { Query[Error] | _ | { "error during a query" }, Key[Error] | _ | {
    ...
    format!("tx response event consists of an error: {}", e.event) }, TendermintOnly | _
    | { "only Tendermint clients can be upgraded" } }
}
#[derive(Clone, Debug)]
...
pub fn build_and_send_ibc_upgrade_proposal(
    ...
    let upgraded_client_state = client_state
        .upgrade(
